...
from reality_sr.utils.color import rgb_to_ycbcr_torch, bgr_to_ycbcr
...
def _estimate_aggd_parameters(vector: np.ndarray) -> [np.ndarray, float, float]:
    r"""Python implements the NIQE (Natural Image Quality Evaluator) function,
...
class _ExactPadding2d(nn.Module):
...
def _image_filter(tensor: torch.Tensor,
    ...
    exact_padding_2d = _ExactPadding2d(kernel_size, stride, dilation, mode=padding)
